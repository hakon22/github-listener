...
- **Логические изменения**:
  - **Схемы сущностей и их использование** — при изменении схемы ORM‑сущности (например, добавлено поле в `UserEntity`) детектор в `CodeAnalyzerService` фиксирует изменение (`logical-entity-schema-change`). Обобщённое сообщение «проверьте все места» в отчёт не попадает; вместо этого `AIService.getEntitySchemaChangeIssues`:
    - находит файлы, где используется сущность: по графу импортов (`collectCallerFilePaths` + pathResolver из tsconfig) и при отсутствии совпадений — по подстроке (имя сущности, новые поля); для анализа используется до 500 файлов (`getSourceFilePathsForEntityUsage`);
    - нумерует строки и передаёт контекст в модель (`entitySchemaChangePrompt`);
    - формирует только конкретные проблемы по файлам/строкам (`logical-entity-usage`). В «Топ проблем» выводятся до 5 пунктов; для каждого при необходимости подгружается содержимое файла по API, чтобы показать сниппет кода (в т.ч. для файлов, не входящих в коммит).
  - **Сигнатуры функций/методов** — изменения параметров и вызовов.
  - **Загрузка данных** — в зависимости от `USE_UNIFIED_AI_ANALYSIS`: при `true` выполняется **единый AI‑анализ** изменённых файлов (один запрос к модели); при `false` — полный пайплайн с traceability, call sites и проверкой загрузки полей/связей (TypeORM, Knex, Prisma и т.д.). В обоих случаях модель ищет обращения к полям/связям, которые запрос не подгружает.
