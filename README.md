...
- **Логические изменения** — изменения сигнатур и схем сущностей; проверка загрузки данных выполняется **моделью ИИ**: по изменённому коду модель ищет места, где результат запроса (TypeORM, Knex, Prisma, raw SQL и т.д.) используется с обращением к полям/связям, которые запрос не подгружает.
